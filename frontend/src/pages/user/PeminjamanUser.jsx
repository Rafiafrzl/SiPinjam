...
import { IoList, IoCalendar, IoTime, IoCheckmarkCircle, IoClose } from 'react-icons/io5';
    ...
    <div className="space-y-6">
      ...
      {peminjaman.length === 0 ? (
        ...
        <div className="space-y-4">
          {peminjaman.map((item) => (
            <Card key={item._id} hover onClick={() => handleShowDetail(item._id)}>
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-gray-600">
                      <IoCheckmarkCircle />
                      <span>
                        Kembali: {format(new Date(item.tanggalKembali), 'dd MMM yyyy', { locale: id })}
                      </span>
                    </div>
                  </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{selectedPeminjaman.barangId?.kategori}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {selectedPeminjaman.status === 'disetujui' && (
                <div className="flex justify-between">
                  ...
                  {getStatusPengembalianBadge(selectedPeminjaman.statusPengembalian)}
                </div>
              )}
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {format(new Date(selectedPeminjaman.tanggalPinjam), 'dd MMMM yyyy', { locale: id })}
                </span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {format(new Date(selectedPeminjaman.tanggalKembali), 'dd MMMM yyyy', { locale: id })}
                </span>
              </div>

              <div className="pt-3 border-t">
                <p className="text-gray-600 mb-2">Alasan Peminjaman:</p>
                <p className="text-gray-800">{selectedPeminjaman.alasanPeminjaman}</p>
              </div>
              ...
              {selectedPeminjaman.catatanAdmin && (
                <div className="pt-3 border-t bg-blue-50 p-3 rounded">
                  ...
                  <p className="text-gray-800">{selectedPeminjaman.catatanAdmin}</p>
                </div>
              )}

              {selectedPeminjaman.alasanPenolakan && (
                <div className="pt-3 border-t bg-red-50 p-3 rounded">
                  <p className="text-gray-600 mb-1 text-sm">Alasan Penolakan:</p>
                  <p className="text-red-800">{selectedPeminjaman.alasanPenolakan}</p>
                </div>
              )}
            </div>
